...
app.post("/verify-code", async (req, res) => {
    try {
        ...
        const isAdminCode = code === process.env.ADMIN_CODE;
...
app.post("/random-name-code", verifyAdminToken, async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Could not generate name-change code."
        });
    }
});
...
app.post("/change-username", verifyUserToken, async (req, res) => {
    try {
        ...
        if (!code) {
            return res.status(400).json({
                success: false,
                message: "Enter a name-change code."
            });
        }
        ...
        if (error.code === "INVALID_NAME_CODE") {
            return res.status(400).json({
                success: false,
                message:
                    "Invalid or already-used name-change code."
        ...
        if (error.code === "EXPIRED_NAME_CODE") {
            return res.status(400).json({
                success: false,
                message:
                    "That name-change code has expired."
    ...
    try {
        ...
        if (
            ...
            return res.status(403).json({
                success: false,
                muted: true,
                message: "The chat is currently muted."
            });
        }
...
app.post("/send-whisper", verifyUserToken, async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Could not send whisper."
        });
    }
});
